	$(document).ready(function() {
		...
		$('#scZipBtn').click(function() {
			...
			if ($('#scZipText').val() == '') {
				$('#scZipCheck').html("<font color=red size='2px' style='font-weight:bold'> 도로명 주소를 입력해 주세요. </font>").show();
			} else {
				$.ajax({
					chche : false,
					async : false,
					...
					success : function(json) {
						...
						if(json.zip.length == 0){
							...
							$('#scZipCheck').html("<font color=red size='2px' style='font-weight:bold'> 도로명 주소가 없습니다</font>").show();
