...
<div class="box01">
	<div>
		...
		<div class="btn_area">
			<input type="submit" class="btn_agree" value="AGREE" onclick="return init.termsVaild();">
...
var init = {
		termsEvent: function() {
		    ...
		    $(':checkbox:first', tbl).click(function(){
		        ...
		        // 모든 체크박스에 change 이벤트 발생시키기(CSS 설정 시 사용)
		    ...
		    // cstEmailAgreement의 hidden value 변경하기
		     $('input:checkbox[name=cstEmailAgreementCheck]').change(function(){
		    	if($(this).is(':checked')){
		    		$('input:hidden[name=cstEmailAgreement]').val('T');
		    	}else{
		    		$('input:hidden[name=cstEmailAgreement]').val('F');
		...
		, termsVaild: function() {
			/* 체크박스 필수선택 에러메시지 */
			if( ($('#serviceAgreementCheck').is(':checked') == false) || ($('#privacyAgreementCheck').is(':checked') == false) ){
				$('.checkbox_valid').show();
				return false;
			} else {
				$('.checkbox_valid').hide();
